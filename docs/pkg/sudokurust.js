...
export class SolveResult {
     ...
     */
    get board() {
        const ret = wasm.solveresult_board(this.__wbg_ptr);
        let v1;
        if (ret[0] !== 0) {
     ...
     */
    get call_cnt() {
        const ret = wasm.solveresult_call_cnt(this.__wbg_ptr);
        return ret >>> 0;
    }
    /**
     * @returns {boolean}
     */
    get solved() {
        const ret = wasm.solveresult_solved(this.__wbg_ptr);
        return ret !== 0;
    }
}
 ...
 * @returns {SolveResult}
 */
export function solve_wasm(input) {
    const ptr0 = passArray8ToWasm0(input, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.solve_wasm(ptr0, len0);
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('sudokurust_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
